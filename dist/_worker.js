...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260504-reserve-v18"}));
...
<div class="warn">⚠️ ログイン後すぐにパスワードを変更してください<br>このURLは設定後も有効なため、token を知らない人には教えないでください</div>
  ...
  for(const base of [...new Set(bases)]){
    ...
    if(!o.ok){
      ...
      else if(o.status===403)hint="（403 Forbidden: 投稿権限なし。X Developer Portal の User authentication settings を Read and write に変更後、Access Token & Secret を Regenerate し、API設定とアカウント管理を保存し直してください。build:fix-20260504-reserve-v18）";
...
`).trim();return s=insertBreaks20(s),s=jn(s),t==="140"&&s.length>140&&(s=s.slice(0,137)+"..."),s}function insertBreaks20(text){if(!text)return"";const weight=s=>[...s].reduce((a,ch)=>a+(ch.charCodeAt(0)>255?2:1),0),flush=(out,buf)=>{const v=buf.trim();v&&out.push(v)},splitLine=line=>{const parts=[];let buf="";for(const ch of line){buf+=ch;const w=weight(buf);if(w>=34&&/[、，,]/.test(ch)){flush(parts,buf);buf="";continue}if(w>=44&&/[。！？!?]/.test(ch)){flush(parts,buf);buf="";continue}if(w>=56){flush(parts,buf);buf=""}}flush(parts,buf);return parts};const raw=text.replace(/\r\n/g,"\n").replace(/\r/g,"\n").split("\n"),out=[];let lineCount=0;for(const ln of raw){const line=ln.trim();if(!line){if(out.length&&out[out.length-1]!=="")out.push("");lineCount=0;continue}if(/^https?:\/\//i.test(line)||/^#/.test(line)||/^@/.test(line)){if(lineCount>=2&&out[out.length-1]!=="")out.push("");out.push(line);lineCount=0;continue}for(const part of splitLine(line)){out.push(part);lineCount++;if(lineCount>=2){out.push("");lineCount=0}}}while(out.length&&out[out.length-1]==="")out.pop();return out.join("\n").replace(/\n{3,}/g,"\n\n").trim();}function jn(e){if(!e)return"";const t=e.split(`
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v18 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
...
function __xCleanTweetId(e){return String(e||"").trim().replace(/[^0-9]/g,"")}
at.post("/api/admin/thread/post-now",m,async e=>{
  ...
  const tweets=Array.isArray(arr)?arr.map(it=>({body:String((it&&it.body)||"").trim(),media_ids:Array.isArray(it&&it.media_ids)?it.media_ids:[]})).filter(it=>it.body.length>0):[];
