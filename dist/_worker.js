...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260504-reserve-v19"}));
...
<div class="warn">⚠️ ログイン後すぐにパスワードを変更してください<br>このURLは設定後も有効なため、token を知らない人には教えないでください</div>
  ...
  for(const base of [...new Set(bases)]){
    ...
    if(!o.ok){
      ...
      else if(o.status===403)hint="（403 Forbidden: 投稿権限なし。X Developer Portal の User authentication settings を Read and write に変更後、Access Token & Secret を Regenerate し、API設定とアカウント管理を保存し直してください。build:fix-20260504-reserve-v19）";
...
async function __xPostTweetStrict(creds,text,mediaIds,replyToId){
  ...
  const chunks=__xSplitTweetText(clean,250);
...
改行を含めて140文字以内のX投稿を作成。1行20文字程度で読みやすくし、簡潔かつインパクト重視。ハッシュタグは含めない。`:n+=`
X投稿用のフル文章を作成。1行20文字程度で自然に改行を入れる。ハッシュタグは含めない。`,e.cta&&(n+=`
CTA: ${e.cta}`),e.userInput&&(n+=`
追加指示: ${e.userInput}`);const i=t+a+s;return{messages:[{role:"system",content:i},{role:"user",content:n}],systemPrompt:i,userPrompt:n}}async function Hs(e,t,s,a,n,i="body"){const{messages:r}=Ps({theme:t,keywords:s,brandVoice:n,targetDna:a,postMode:i||"body"}),o=await qs(r,{apiKey:e,temperature:.8});return bt(o,i)}async function Us(e,t,s,a,n,i,r="body"){if(!Rt[t])throw new Error(`未対応のパターン: ${t}`);const{messages:o}=Ps({theme:s,keywords:a,brandVoice:i,targetDna:n,patternType:t,postMode:r||"body"}),d=await qs(o,{apiKey:e,temperature:.8});return bt(d,r)}function __limitChars(text,max){const arr=[...String(text||"")];if(arr.length<=max)return String(text||"").trim();const keep=Math.max(0,max-3);return arr.slice(0,keep).join("").replace(/\s+$/g,"")+"..."}function bt(e,t){if(!e)return"";let s=e.replace(/^#{1,4}\s*/gm,"").replace(/^[▪️■●•・\-\*]+\s*/gm,"").replace(/^\d+\.\s/gm,"").replace(/^(Step\d+)[:\s]/gim,"").replace(/^[①②③④⑤⑥⑦⑧⑨⑩]\s*/gm,"").replace(/\*\*([^*]+)\*\*/g,"$1").replace(/\n{3,}/g,`\n\n`).trim();if(t==="140"){s=__limitChars(s.replace(/\s+/g," ").trim(),140);s=insertBreaks20(s);return __limitChars(s,140)}return jn(insertBreaks20(s))}function insertBreaks20(text){if(!text)return"";const out=[],push=buf=>{const v=String(buf||"").trim();v&&out.push(v)},splitLine=line=>{let buf="";for(const ch of [...line]){buf+=ch;const n=[...buf].length;if(n>=16&&/[、，,。！？!?]/.test(ch)){push(buf);buf="";continue}if(n>=20){push(buf);buf=""}}push(buf)};for(const raw of String(text).replace(/\r\n/g,"\n").replace(/\r/g,"\n").split("\n")){const line=raw.trim();if(!line){if(out.length&&out[out.length-1]!=="")out.push("");continue}if(/^https?:\/\//i.test(line)||/^#/.test(line)||/^@/.test(line)){out.push(line);continue}splitLine(line)}while(out.length&&out[out.length-1]==="")out.pop();return out.join("\n").replace(/\n{3,}/g,"\n\n").trim()}function jn(e){if(!e)return"";const t=e.split(`
...
let replyToId=null;if(r.thread_parent_id){const tp=String(r.thread_parent_id);if(tp.startsWith("prev:")){const prevId=parseInt(tp.slice(5),10);if(prevId){const prev=await e.env.DB.prepare("SELECT external_post_id,status,error_message FROM post_queue WHERE id=? AND user_id=?").bind(prevId,r.user_id).first();if(prev&&prev.external_post_id)replyToId=prev.external_post_id;else if(prev&&["failed","cancelled","rejected"].includes(prev.status)){await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind("thread_parent_failed: "+(prev.error_message||prev.status),g(),r.id).run();continue}else{await e.env.DB.prepare("UPDATE post_queue SET status='scheduled', updated_at=? WHERE id=?").bind(g(),r.id).run();continue}}}else if(/^\d+$/.test(tp)){const prev=await e.env.DB.prepare("SELECT external_post_id,status,error_message FROM post_queue WHERE id=? AND user_id=?").bind(Number(tp),r.user_id).first();if(prev&&prev.external_post_id)replyToId=prev.external_post_id;else if(prev&&["failed","cancelled","rejected"].includes(prev.status)){await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind("thread_parent_failed: "+(prev.error_message||prev.status),g(),r.id).run();continue}else replyToId=tp}}const b=replyToId?(_.length>0?await $sReply(c,p,replyToId,_):await $sReply(c,p,replyToId)):(_.length>0?await $s(c,p,_,null):await Ms(c,p));const bx=(b&&b.id)||"",btail=(b&&b.last_id)||bx,bstore=r.post_mode==="thread"?btail:bx;await e.env.DB.prepare("UPDATE post_queue SET status='posted', external_post_id=?, posted_at=?, updated_at=? WHERE id=?").bind(bstore,g(),g(),r.id).run(),
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v19 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
...
at.post("/api/admin/thread/post-now",m,async e=>{
  ...
  if(tweets.length>20)return e.json({success:!1,error:"max 20 replies"},400);
  ...
  for(let i=0;i<tweets.length;i++){
    ...
    try{
      ...
      errs.push({index:i,error:(err&&err.message)||"post_failed",parent_id:parent||""});
      break;
    }
  }
  ...
  for(let i=0;i<tweets.length;i++){
    ...
    const r=await e.env.DB.prepare(`INSERT INTO post_queue
       ...
       VALUES ('x', ?, ?, ?, 'thread', ?, ?, 'scheduled', 'thread', ?, ?, ?, ?, ?, ?, ?)`).bind(
        t.id, acct.id, it.body, sched, sched, i===0?(tid||null):"prev:"+(ids[i-1]||""), i, tweets.length,
