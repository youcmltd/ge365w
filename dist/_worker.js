...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260503-reserve-v13"}));
...
<div class="warn">⚠️ ログイン後すぐにパスワードを変更してください<br>このURLは設定後も有効なため、token を知らない人には教えないでください</div>
  ...
  for(const base of [...new Set(bases)]){
    ...
    if(!o.ok){
      ...
      else if(o.status===403)hint="（403 Forbidden: 投稿権限なし。X Developer Portal の User authentication settings を Read and write に変更後、Access Token & Secret を Regenerate し、API設定とアカウント管理を保存し直してください。build:fix-20260503-reserve-v13）";
...
async function __xPostTweetStrict(creds,text,mediaIds,replyToId){
  const errors=[];
  if(creds&&creds.oauth2AccessToken){
    try{return await xOAuth2PostTweet(creds,text,mediaIds,replyToId)}
    catch(e){errors.push("OAuth2="+(((e&&e.message)||String(e)).slice(0,700)))}
  }
  try{return await __xOAuth1PostTweetApiX(creds,text,mediaIds,replyToId)}
  catch(e){errors.push("OAuth1="+(((e&&e.message)||String(e)).slice(0,700)));throw new $(`v13投稿失敗: ${errors.join(" / ")}。空POSTによる投稿権限判定は廃止しました。API設定の「投稿権限テスト（投稿→削除）」または実投稿と同じ経路で確認してください。`,e instanceof $?e.statusCode:0,"api_error_strict_post")}
}
...
`+c.trim());const nt=await Ae(L),Q=await e.env.DB.prepare(`INSERT INTO post_queue
  ...
  try{
    ...
    const msg="v13 post-now account="+i.id+" @"+(i.x_username||i.account_name||"")+": "+(d.message||String(d));
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v13 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
