...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260503-reserve-v15"}));
...
<div class="warn">⚠️ ログイン後すぐにパスワードを変更してください<br>このURLは設定後も有効なため、token を知らない人には教えないでください</div>
  ...
  for(const base of [...new Set(bases)]){
    ...
    if(!o.ok){
      ...
      else if(o.status===403)hint="（403 Forbidden: 投稿権限なし。X Developer Portal の User authentication settings を Read and write に変更後、Access Token & Secret を Regenerate し、API設定とアカウント管理を保存し直してください。build:fix-20260503-reserve-v15）";
...
async function __xPostTweetStrict(creds,text,mediaIds,replyToId){
  ...
  if(chunks.length<=1)return await __xPostOneWithRetry(creds,clean,mediaIds,replyToId);
  let parent=replyToId||null,first=null;
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v15 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
...
at.post("/api/admin/thread/post-now",m,async e=>{
  ...
  if(!Array.isArray(arr)||arr.length===0)return e.json({success:!1,error:"tweets are required"},400);
  ...
  for(let i=0;i<arr.length;i++){
    ...
    const r=await e.env.DB.prepare(`INSERT INTO post_queue
       ...
       VALUES ('x', ?, ?, ?, 'thread', ?, ?, 'scheduled', 'thread', ?, ?, ?, ?, ?, ?, ?)`).bind(
        t.id, acct.id, it.body, sched, sched, i===0?(tid||null):"prev:"+(ids[i-1]||""), i, arr.length,
