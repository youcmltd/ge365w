...
<div class="space-y-4">
  ...
  <div class="card space-y-4">
    ...
    <div>
      ...
      <input type="password" id="api-xoauth2" class="inp input-mono" placeholder="${t.oauth2_user_token_set?"設定済み（変更する場合のみ入力）":"任意: OAuth2のUser Access Token"}" value="">
      <div style="font-size:.75rem;color:var(--ink-muted);margin-top:.25rem">OAuth1が403の場合、このTokenを優先してPOST /2/tweetsに使います。必要scope: tweet.write / tweet.read / users.read</div>
    </div>
    <div style="display:flex;gap:.5rem;align-items:center;flex-wrap:wrap">
...
async function saveXApi(){
  const r=await fetch('/api/admin/api-settings/x',{method:'POST',headers:{'content-type':'application/json'},
    body:JSON.stringify({api_key:document.getElementById('api-xk').value,api_secret:document.getElementById('api-xs').value,oauth2_user_token:document.getElementById('api-xoauth2')?document.getElementById('api-xoauth2').value:''})});
  const j=await r.json();
...
<div class="warn">⚠️ ログイン後すぐにパスワードを変更してください<br>このURLは設定後も有効なため、token を知らない人には教えないでください</div>
  ...
  for(const base of [...new Set(bases)]){
    ...
    if(!o.ok){
      ...
      else if(o.status===403)hint="（403 Forbidden: 投稿権限なし。X Developer Portal の User authentication settings を Read and write に変更後、Access Token & Secret を Regenerate し、API設定とアカウント管理を保存し直してください。build:fix-20260503-reserve-v10）";
...
Ft=async function(env,account,override){
  ...
  let oauth2AccessToken="";try{const enc=await env.DB.prepare("SELECT value FROM system_settings WHERE key=?").bind("x_oauth2_user_token").first();if(enc&&enc.value)oauth2AccessToken=(await At(enc.value,env.ENCRYPTION_KEY)).trim()}catch{}return{consumerKey:pair.apiKey,consumerSecret:pair.apiSecret,accessToken,accessTokenSecret,oauth2AccessToken};
};
...
`+c.trim());const nt=await Ae(L),Q=await e.env.DB.prepare(`INSERT INTO post_queue
  ...
  try{
    ...
    const resolveReply=async row=>{if(!row.thread_parent_id)return null;const tp=String(row.thread_parent_id);if(tp.startsWith("prev:")){const prevId=parseInt(tp.slice(5),10);if(prevId){const prev=await e.env.DB.prepare("SELECT external_post_id FROM post_queue WHERE id=? AND user_id=?").bind(prevId,t.id).first();if(prev&&prev.external_post_id)return prev.external_post_id;throw new Error("Parent reply is not posted yet (post_queue id="+prevId+")")}}else if(/^\d+$/.test(tp)){const prev=await e.env.DB.prepare("SELECT external_post_id FROM post_queue WHERE id=? AND user_id=?").bind(Number(tp),t.id).first();return prev&&prev.external_post_id?prev.external_post_id:tp}return null};
    const postRow=async(row,replyTo)=>{await e.env.DB.prepare("UPDATE post_queue SET status='publishing', account_id=COALESCE(account_id,?), updated_at=? WHERE id=? AND user_id=?").bind(i.id,g(),row.id,t.id).run();const mids=await mediaIds(row),txt=makeText(row),out=replyTo?(mids.length>0?await $sReply(d,txt,replyTo,mids):await $sReply(d,txt,replyTo)):(mids.length>0?await $s(d,txt,mids,null):await Ms(d,txt));await e.env.DB.prepare("UPDATE post_queue SET status='posted', external_post_id=?, posted_at=?, updated_at=? WHERE id=?").bind(out.id||"",g(),g(),row.id).run();await ns(e.env,{record_id:row.id,account_id:i.id,user_id:t.id,account_name:i.account_name,source_type:row.source_type||"manual_post",generation_type:row.generation_type,post_mode:row.post_mode==="thread"?"thread":row.post_mode,content:row.body||"",content_hash:row.content_hash||"",link_url:row.link_url,thread_parent_id:replyTo||row.thread_parent_id||null,thread_order:row.thread_order??null,thread_total_count:row.thread_count??null,posted_at:g(),status:"posted",api_response_summary:JSON.stringify({tweet_id:out.id})});postedIds.push(out.id||"");return out.id||replyTo};
    let rows=[n];
    ...
    for(const row of rows){replyTo=await postRow(row,replyTo)}
    ...
    const msg="v10 post-now account="+i.id+" @"+(i.x_username||i.account_name||"")+": "+(d.message||String(d));
    await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(msg,g(),s).run();
    await ns(e.env,{record_id:s,account_id:i.id,user_id:t.id,account_name:i.account_name,source_type:n.source_type,post_mode:n.post_mode,content:n.body||"",content_hash:n.content_hash||"",status:"failed",error_message:msg});
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v10 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
