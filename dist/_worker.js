...
<\/script>`}function pn(e){return`
    ...
    </div>`}function mn(e){const t=JSON.stringify(e.scheduled||[]).replace(/`/g,"\\`").replace(/<\/script>/gi,"<\\/script>");return`
<div class="space-y-4">
        ...
        <tbody id="sc-list-body">
          ${e.scheduled.length===0?'<tr><td colspan="5" style="text-align:center;color:var(--ink-muted);padding:2.5rem">予約なし</td></tr>':e.scheduled.map(s=>`
              <tr>
                ...
                <td style="font-size:.75rem">@${w(s.x_username||"-")}</td>
                <td style="font-size:.75rem;max-width:20rem"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${w((s.body||"").slice(0,80))}</div></td>
                <td>${Lt(s.status)}</td>
...
showSection('users');
<\/script>
`;return e.html(It("管理画面",t))});F.get("/api/admin/users",m,R,async e=>{const t=e.req.query("filter")||"all",s=[];t==="pending"&&s.push("u.is_approved = 0"),t==="approved"&&s.push("u.is_approved = 1"),t==="admin"&&s.push("u.is_admin = 1");const a=`
