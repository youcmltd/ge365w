      ...
      <tbody>
        ${n.length===0?'<tr><td colspan="8" class="text-center text-ink-muted py-10">この月の投稿データがありません</td></tr>':n.map(r=>`
            <tr data-status="${r.status||""}" data-scheduled="${r.scheduled_at?"1":"0"}" class="post-row">
              ...
              <td class="max-w-md"><div class="truncate">${w((r.body||"").slice(0,80))}</div>${r.error_message?`<div style="font-size:.72rem;color:#dc2626;margin-top:.25rem;white-space:normal">⚠ ${w(r.error_message)}</div>`:""}</td>
...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260503-reserve-v7"}));
...
async function __postTweetWithFallback(creds,text,mediaIds,replyToId){try{if(replyToId)return mediaIds&&mediaIds.length>0?await $sReplyV2(creds,text,replyToId,mediaIds):await $sReplyV2(creds,text,replyToId);return mediaIds&&mediaIds.length>0?await $sV2(creds,text,mediaIds,null):await MsV2(creds,text)}catch(err){if(!__xShouldFallbackV11(err))throw err;const params={status:text};if(mediaIds&&mediaIds.length>0)params.media_ids=mediaIds.slice(0,4).join(",");if(replyToId){params.in_reply_to_status_id=replyToId;params.auto_populate_reply_metadata="true"}try{return await xV11_statusUpdate(creds,params)}catch(err2){const msg=`v7 fallback実行: v2投稿失敗 -> v1.1投稿も失敗。v2=${err.message} / v1.1=${err2.message}`;throw new $(msg,err2 instanceof $?err2.statusCode:err.statusCode,"api_error_both")}}}
...
`+c.trim());const nt=await Ae(L),Q=await e.env.DB.prepare(`INSERT INTO post_queue
  ...
  try{
    ...
    const msg="v7 post-now account="+i.id+" @"+(i.x_username||i.account_name||"")+": "+(d.message||String(d));
...
await e.env.DB.prepare(`UPDATE x_accounts SET
           ...
           ON CONFLICT(account_id, metric_date) DO UPDATE SET
             ...
             updated_at = datetime('now','+9 hours')`).bind(d.id,r.user_id).run(),n++}finally{await Js(e.env,d.id)}}catch(d){const l="v7 cron account="+(r.account_id||"")+": "+((d==null?void 0:d.message)||"unknown_error");await e.env.DB.prepare("UPDATE post_queue SET status='failed', error_message=?, updated_at=? WHERE id=?").bind(l,g(),r.id).run(),
