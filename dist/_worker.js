...
F.get("/api/server-time",e=>e.json({now:g(),now_ms:Date.now(),build:"fix-20260503-reserve-v4"}));
...
const d=acctRow;const l=await Ks(e.env,d.id,r.body||"",r.link_url,r.hashtags);if(!l.ok)throw new Error("safety: "+l.errors.map(c=>c.message).join("; "));if(!await Ys(e.env,d.id)){
  ...
  continue;
}try{const c=await Ft(e.env,d);await __xWritePreflight(c);let p=bt(r.body||"",r.post_mode);r.link_url&&(p+=`
...
if(job.generated_post_id){
  await e.env.DB.prepare("UPDATE post_queue SET status='scheduled', scheduled_at=?, effective_scheduled_at=?, base_scheduled_at=?, error_message=NULL, external_post_id=NULL, posted_at=NULL, updated_at=? WHERE id=? AND user_id=?").bind(newPublishAt,newPublishAt,newPublishAt,g(),job.generated_post_id,t.id).run();
